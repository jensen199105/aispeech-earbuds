...
#define COMPLEX_MUL_FLOAT_SQ(Z,A,B, Q) \
    do { \
        (Z).real = MULT32_32_SQ((A).real , (B), Q); \
        (Z).imag = MULT32_32_SQ((A).imag , (B), Q); \
    } while (0)
#else
#define COMPLEX_MUL_FLOAT(Z,A,B) \
    do { \
...
#define COMPLEX_CONJ_MUL_SQ(Z,A,B, Q) \
    do { \
        ...
        aisp_s32_t tmpR = PSHR((ARBR + AIBI), Q); \
        aisp_s32_t tmpI = PSHR((AIBR - ARBI), Q); \
        Z.real = tmpR; \
        Z.imag = tmpI; \
    } while (0)
...
#if 0
#define COMPLEX_POW_SQ(Z,A, Q) \
    do \
    { \
        aisp_s64_t ZZ_ = PSHR64((MULT32_32((A).real, (A).real) + MULT32_32((A).imag , (A).imag)), Q);\
        if(ZZ_> 2147483648){printf("%s %s [%d] +++++\r\n", __FILE__, __func__, __LINE__);} \
        (Z) = ZZ_; \
    } while (0)
#else
#define COMPLEX_POW_SQ(Z,A, Q) \
    do \
    { \
        (Z) = PSHR64((MULT32_32((A).real, (A).real) + MULT32_32((A).imag , (A).imag)), Q); \
    } while (0)
...
#define COMPLEX_POW(Z,A) \
    do \
    { \
        (Z) = (A).real * (A).real + (A).imag * (A).imag; \
    } while (0)

#define COMPLEX_DIV_FLOAT(Z,A,B) \
    do { \
        (Z).real = (A).real / ((B)); \
        (Z).imag = (A).imag / ((B)); \
    } while (0)
