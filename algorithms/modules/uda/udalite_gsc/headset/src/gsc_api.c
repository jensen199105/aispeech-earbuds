...
S32 GSC_UnsharedMemSizeGet(GSC_CoreCfg_S *pstGscCfg)
{
    ...
    iTotalMemSize  = AISP_ALIGN_SIZE(iTotalMemSize, AISP_ALIGN_8_MASK);
    /* pcpxGscX buf size */
    iTotalMemSize += iGscNum * GSC_FFTBINS * iTaps * sizeof(GSC_COMPLEX_T);
    iTotalMemSize  = AISP_ALIGN_SIZE(iTotalMemSize, AISP_ALIGN_8_MASK);
              ...
              void *pvUsrData)
{
    ...
    pstGscApi->psHanningWind      = (GSC_S32_T *)g_aHanningWindow; /* FFT hanning window */
...
GSC_VISIBILITY_ATTR S32 GSC_WindCbRegister(GSC_Api_S *pstGscApi, void *pvHandler, void *pvData)
{
    pstGscApi->pfunWindCb = (GSC_FUNCHOOK_T)pvHandler;
    pstGscApi->pParm      = pvData;
    return 0;
}
...
GSC_VISIBILITY_ATTR S32 GSC_Feed(GSC_Api_S *pstGscApi, S8 *pcData, S32 iLen)
{
    ...
    if (!iRet)
    {
        ...
        /* data output */
        dataPop(pstGscApi, pstGscApi->pstGscInstance->pcpxXFFT);
    }
